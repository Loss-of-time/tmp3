...
- **剔除高溢价**：QDII 溢价 >3% 不要（剔除纳指+12.5%/美国50+5.9%/日经+4.1%；恒生科技 430亿 溢价≈0 保留）
...
- **参数敏感性** `etf_rot_signal_sweep.py`（v7 版逐参数扰动）：唯一陡峭旋钮是 `mom_gap`（0→7.7%、0.5→12.6%、1.0→26.1%，即切换门槛是策略核心）；其余呈平台：ma_n 200/220、lookback 150-180、trail 0.20-0.25 为峰区；base_w 单调权衡（低底仓高收益高回撤）；cooldown/min_mom 不敏感。26.1% 非单点过拟合。
